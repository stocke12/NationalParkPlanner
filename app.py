    ...
    with t1:
        ...
        if st.button("Login"):
            with engine.connect() as conn:
                # FIX: Use .mappings() to allow dictionary-style access (user_info['likes'])
                res = conn.execute(text("SELECT id, username, firstname, lastname, email, likes FROM users WHERE username = :u"), {"u": u}).mappings().fetchone()
                if res:
...
# --- MAIN APP ---
    ...
    with plan_tab:
        ...
        # KEY: 'invited_friends' ensures selection persists in st.session_state
        st.multiselect("Invite Friends?", options=friend_names, key="invited_friends")

        if st.button("Generate Plan"):
            if len(date_range) < 2: 
                st.error("Select a range.")
            else:
                nights = (date_range[1] - date_range[0]).days
                # FIX: Access by key 'likes'
                prompt = f"""
    ...
    with my_trips_tab:
        with engine.connect() as conn:
            trips = conn.execute(text("""
                SELECT t.trip_name, tpk.notes, t.id 
                FROM trips t 
                JOIN trip_parks tpk ON t.id = tpk.trip_id 
                WHERE t.user_id = :u
            """), {"u": current_uid}).fetchall()
            for t in trips:
                with st.expander(f"📍 {t[0]}"):
                    st.markdown(t[1])
                    pdf_b = create_pdf(t[1], t[0], st.session_state.user_info['firstname'])
